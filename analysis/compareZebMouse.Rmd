...
- Out of the 3, 6, and 12 mouse age groups, which is most similar to 
6 month mutant zebrafish?

```{r}
butterfly <- bind_rows(zeb_facet1, mou2_facet1, mou2_facet2,mou2_facet3) %>%
...
```{r}
butterfly2 <- bind_rows(zeb_facet1, zeb_facet2, mou2_facet2,mou2_facet3) %>%
  dplyr::mutate(Geneset = gsub(x = Geneset, pattern = "_", replacement = " "),
                Geneset = gsub(x = Geneset, pattern = "HALLMARK", 
                               replacement = "")) %>%
  reshape2::melt(id.vars=c("Geneset","NGenes","contrast", "significant")) %>%
