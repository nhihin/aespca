...
- t-SNE is a dimension reduction method that can be used to visualise 
structure within samples similar to PCA. However, while PCA is linear, 
t-SNE is non-linear. We will try this to see if we can see better 
separation of samples. 

- t-SNE is influenced greatly by parameter choice, particularly the 
perplexity, which is usually a value < 50. Here we will try several 
to see which might work better for the data. 

- Note: Higher perplexity means more global structure, lower perplexity 
to prioritise more local structure.
...
heatmapAnnot <- testclust2%>% as.data.frame%>%
  set_rownames(.$Sample) %>%
